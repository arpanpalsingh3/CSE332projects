...
public class NGramToNextChoicesMap {
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        ...
        Comparator<Item<String, Integer>> comp = new LargeValueFirstItemComparator<>();
        if (k < 0) {
            ...
            Item<String, Integer>[] reorder = new Item[afterNGrams.length];
            if (k > afterNGrams.length) {
                k = afterNGrams.length;
            }
            for (int i = 0; i < k; i++) {
                reorder[i] = afterNGrams[k - 1 - i];
            }
            afterNGrams = reorder;
        }
